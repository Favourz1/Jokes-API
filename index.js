...
app.get("/filter", (req, res) => {
  ...
  const requestedJoke = jokes.filter((joke) => joke.jokeType.toLowerCase() == type.toLowerCase())
  res.status(200).json(requestedJoke)
})
...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeBody.text) {
    res.status(404).send("<h1> Text property is required </h1>")
    return;
  }
  if (!jokeBody.type) {
    res.status(404).send("<h1> Type property is required </h1>")
    return;
  }
  jokes.push({
    id: jokes.length + 1,
    jokeText: jokeBody?.text,
    jokeType: jokeBody.type
  })
  res.status(201).json(jokes[jokes.length - 1])
})
...
app.put("/jokes/:jokeId", (req, res) => {
  ...
  res.status(201).json(jokes[jokeIndex])
})
...
app.patch("/jokes/:jokeId", (req, res) => {
  ...
  const jokeIndex = jokes.findIndex(joke => joke.id == id)
  if (!jokeBody) {
    res.status(404).send("At least one key value pair required");
    return;
  }
  jokes[jokeIndex] = {
    ...jokes[jokeIndex],
    id: id,
    jokeText: jokeBody?.text || jokes[jokeIndex].jokeText,
    jokeType: jokeBody?.type || jokes[jokeIndex].jokeType
  }
  res.status(201).json(jokes[jokeIndex])
})
...
app.delete("/jokes/:jokeId", (req, res) => {
  ...
  if (jokeIndex !== -1) {
    ...
    // If the joke with the specified ID is not found
